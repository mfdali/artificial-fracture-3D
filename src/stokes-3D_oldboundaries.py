...
def inside(x, on_boundary):
    ...
    values = zcoords
    r = griddata(points, values, (x[0], x[1]), method='nearest')

    return on_boundary and (x[2]> r - r_tol)
...
case_filename = "thin_box_1mm_meshsize_0002"
...
tol = 1E-6
h_avg = 0.001
coordinates_filename = 'thinbox_1mm_0002_coords.csv'

'''
case_filename = "x2_slopev1_2mm_meshsize_0002"
...
tol = 1E-6
h_avg = 1.5000e-03
coordinates_filename = 'slope10N_coords.csv'
...
coordinates_file = path_to_folder + coordinates_filename

# Load surface csv file
xcoords, ycoords, zcoords = load_data(coordinates_file)

#h = MeshHeight()
#h.set_h_values(xcoords, ycoords, zcoords)
...
#bc6 = DirichletBC(W.sub(0),noslip,boundaries,0)
...
file = File(path_to_folder + case_filename + "_griddata_tol"+str(r_tol)+"-u.pvd")
file << u
file = File(path_to_folder + case_filename + "_griddata_tol" + str(r_tol) + "-p.pvd")
file << p
